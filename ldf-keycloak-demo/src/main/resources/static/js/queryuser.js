...
function del(id){
	$.dialog({
	    ...
	    autofocus: true,
		ok: function(){
			$.post("service/user/delete/"+id, function(data){
				if(data.success){
					query();
				}else{
					sticky("删除失败!", "error") 
...
function Batchdelete(){
	...
	if(userIds!=null&&userIds.length<1){
		...
		$.dialog({
		    ...
		    autofocus: true,
			ok: function(){
				$.post("service/user/delete/"+recordIds, function(data){
					if(data.success){
						query();
					}else{
						sticky("删除失败!", "error") 
...
function modify(id){
	var url = "modifyuser.html";
	if(id){
		url += "?id=" + id;
	}
	/*window.parent.updateUrl(url);*/
...
function renderoptions(data, type, full) {
	return '<div>'
			...
			+ '<span>&nbsp;&nbsp</span>' 
			+ '<a data-optCode="modifyuser" onclick="modify(' + "'" + data + "'" + ')">编辑</a>'
			+'</div>'
}
//渲染状态
...
function authzOpt(){
	$("[data-optCode]").each(function(){
		var $opt = $(this);
		var resourceId = $opt.attr("data-optCode");
		$.post("service/home/getOpt/"+resourceId, function(data){
			if(!data){
				$opt.hide();
			}
		});
